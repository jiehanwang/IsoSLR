...
using namespace std;
int saveFiles_va_tra;
const int sg_num=2;
const int SentencesNums=240;
...
// int PostureMatchedIndex[Gallery_num][Posture_num_add][2];
...
void setAll(int saveFiles_va)
{
	saveFiles_va_tra = saveFiles_va;
}
...
int CurveRecognision(float headx0, float heady0, float headz0
	...
	int trajectoryMatchedIndex[][Posture_num_add][2])
{
	for(int i=0;i<num;i++)
	{
		framenum[i]=0;
...
void CurveRecognition(CvPoint3D32f headPoint3D, vector<SLR_ST_Skeleton> vSkeletonData, 
	double score[WordsNum], int sentenceID,int startFrameID, int endFrameID, int trajectoryMatchedIndex[][Posture_num_add][2])
{
	int length = vSkeletonData.size();
	float *lhandx0 = new float[length];
	float *lhandy0 = new float[length];
...
double Sentence_match(CvPoint3D32f probe_head_SG,
	vector<CvPoint3D32f> probe_lhand_SG,
	vector<CvPoint3D32f>probe_rhand_SG,
	CvPoint3D32f gallery_head_SG,
	vector<CvPoint3D32f> gallery_lhand_SG,
	vector<CvPoint3D32f>gallery_rhand_SG)
{
	double d=0;
